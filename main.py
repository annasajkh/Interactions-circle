...
for status in tqdm(tweepy.Cursor(api.user_timeline,screen_name=target_username,tweet_mode="extended").items()):
...
for status in tqdm(tweepy.Cursor(api.get_favorites,screen_name=target_username).items()):
...
urllib.request.urlretrieve(api.get_user(screen_name=target_username).profile_image_url,f"{target_username}.png")

for name, url in user_profil_url.items():
    try:
        if url == None:
            user_profil_url[name] = api.get_user(screen_name=name).profile_image_url
